...
estMin: 20
desc: "Build a real HTTP/1.1 client from the bytes up: parse a URL into its parts, serialize a request to exact raw bytes ending in a blank line, parse a status line and headers back, decode chunked and Content-Length bodies, read multiple responses off one keep-alive stream, follow redirects, carry cookies, and send Basic auth and form-encoded posts - ending in a runnable client that performs a live GET over TCP and prints the response."
